...
' Const sConnectString As String = "DSN = SQLFINMAN;UID = sa;PWD = sa;DSQ = Administration"
...
Sub NewReceipt()
If Not HasRights(CashBook, CanAdd) Then
...
End Sub
Sub EditReceipt()
If Not HasRights(CashBook, CanEdit) Then
  MsgBox "You do not have required privileges to proceed !", vbExclamation
  Exit Sub
End If
Dim AccountID As Long
...
Sub DeleteReceipt()
If Not HasRights(CashBook, CanDelete) Then
    MsgBox "You do not have required privileges to proceed !", vbExclamation
    Exit Sub
End If
...
With crstRpt
    .Reset
    .Connect = m_objConnectDB.sConnect ' sConnectString
    .ReportFileName = App.Path & "\reports\CashBook-2.rpt"
    ...
    ' Get External Values
    Dim cAccts As New cAccounts
    dOB = cAccts.Balance(m_cControlAccounts.AccountNo(CashInHand), dtpDate.value - 1)
    Set cAccts = Nothing
    dtDate = dtpDate.value
    ' Set Up Report
    .ParameterFields(0) = "pmOpeningBalance;" & dOB & ";true"
    .ParameterFields(1) = "pmCompany;" & psCompanyName & ";true"
    .ParameterFields(2) = "pmCashInHand;" & frmControlAccounts.CashInHandAccount & ";true"
